/* ============================================================
   HighBet — ui: иконки, компоненты, тосты, шторки, конфетти
   ============================================================ */
(function () {
  'use strict';
  ...
  ui.statusBadge = function (fx) {
    if (fx.status === 'live') return `<span class="badge badge-live"><span class="dot"></span>LIVE ${fx.minute || ''}'</span>`;
    if (fx.status === 'finished') return `<span class="badge badge-done">Завершён</span>`;
  ...
  ui.fixtureRow = function (fx) {
    ...
    else if (fx.status === 'finished') mid = `<div class="score">${fx.score.home}:${fx.score.away}</div><div class="lbl">итог</div>`;
    else mid = `<div class="time">${HB.fmtTime(fx.kickoff)}</div><div class="lbl">${HB.fmtDay(fx.kickoff)}</div>`;
    return `
      <div class="fixture-head">
        <span class="fixture-league"><span>${lg.emoji}</span> ${lg.name}</span>
        ${fx.status === 'live' ? '<span class="badge badge-live"><span class="dot"></span>LIVE</span>' : ''}
      </div>
      <div class="fixture">
        <div class="fixture-teams">
          <div class="fixture-side home">
            ${ui.crest(fx.homeId)}
            <div style="min-width:0"><div class="tname">${home.name}</div><div class="tcity">${home.city}</div></div>
          </div>
          <div class="fixture-vs">${mid}</div>
          <div class="fixture-side away">
            ${ui.crest(fx.awayId)}
            <div style="min-width:0"><div class="tname">${away.name}</div><div class="tcity">${away.city}</div></div>
          </div>
        </div>
      </div>`;
  };
  ...
  ui.heroFixture = function (fx, midHtml) {
    ...
    return `
      <div class="hero-fixture">
        <span class="fixture-league" style="justify-content:center">${lg.emoji} ${lg.name}</span>
        <div class="hf-teams">
        ...
        <div class="fixture-meta" style="justify-content:center">
          <span class="mi">${ui.icon('pin', { size: 14 })} ${fx.venue || '—'}</span>
  ...
  ui.copy = async function (text) {
    try {
      ...
      try { document.execCommand('copy'); ui.toast('Скопировано', 'ok'); } catch (_) { ui.toast('Не удалось скопировать', 'err'); }
      ta.remove();
    }
  };
